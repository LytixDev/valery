 ...
 */
static bool match_either(unsigned int n, ...)
{
    enum tokentype_t type;
    va_list args;
    va_start(args, n);
...
static void *ast_assignment(void)
{
    if (match_either(1, T_IDENTIFIER)) {
        struct token_t *identifier = previous();
        if (match_either(1, T_EQUAL)) {
...
static void *ast_primary(void)
{
    if (match_either(2, T_STRING, T_NUMBER)) {
